...
def _expand_param(val, default_list):
    """Support list or {min, max, step} dict for hyperparams."""
    if val is None:
        return default_list
...
out_dir = pathlib.Path(out_cfg.get('comparison_csv', 'output/predictions/baseline_elo/baseline_elo_comparison.csv')).parent
...
comp_csv = out_cfg.get('comparison_csv', str(out_dir / 'baseline_elo_comparison.csv'))
k_csv = out_cfg.get('k_metrics_csv', str(out_dir / 'baseline_elo_k_metrics.csv'))
r1_csv = out_cfg.get('round1_csv', str(out_dir / 'round1_baseline_elo_predictions.csv'))
...
summary_json = out_cfg.get('summary_json', str(out_dir / 'baseline_elo_pipeline_summary.json'))
summary = {
    'model': 'BaselineElo',
    'train_ratio': val_cfg.get('train_ratio', 0.7),
    'n_runs': n_folds,
